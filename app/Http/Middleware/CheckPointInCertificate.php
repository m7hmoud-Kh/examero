...
class CheckPointInCertificate
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if($request->plan_id){
            ...
            if($teacher && $teacher->points - $teacher->details_sum_point
            >= TeacherPoint::CERTIFICATE->value)
            {
                $request->merge(['teacher_plans_id'=>$teacher->id]);
                return $next($request);
            }
        }elseif($request->rewarded_point >= TeacherPoint::CERTIFICATE->value){
            return $next($request);
        }
        return response()->json([
            'message' => __('middleware.less_point',[
                'value' => TeacherPoint::CERTIFICATE->value
            ])
