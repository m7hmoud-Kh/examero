...
class AdminPointController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $pointsData = AdminPoint::with(['admin' => function($q) use ($request){
            ...
            'admin_id',
            DB::raw('SUM(CASE WHEN type = '. AdminTypePoint::REWARD->value .' THEN points ELSE 0 END) as reward_points'),
            DB::raw('SUM(CASE WHEN type = '. AdminTypePoint::PUNISHMENT->value .' THEN points ELSE 0 END) as punishment_points'),
            DB::raw('SUM(CASE WHEN type = '. AdminTypePoint::WARNING->value . ' THEN 1 ELSE 0 END) as warning_points')
        )
        ->groupBy('admin_id')
